    ...
    this.tourAuthoringService
      ...
      .subscribe({
        next: (result: PagedResults<AvailableTour>) => {
          // Purchases live in the Purchase microservice; drop tours already bought.
          this.marketplaceService.getTouristOrders(this.user.id).subscribe({
            next: (orders) => {
              const purchased = new Set(orders.results.map((o) => o.tourId));
              this.availableTours = result.results.filter((t) => !purchased.has(t.id));
              this.length = this.availableTours.length;
            },
            error: () => {

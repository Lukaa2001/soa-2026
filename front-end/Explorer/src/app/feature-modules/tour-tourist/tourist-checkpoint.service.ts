...
export class TouristCheckpointService {
  ...
  getCheckpoints(): Observable<PagedResults<Checkpoint>> {
    return this.http.get<PagedResults<Checkpoint>>('http://localhost:8080/api/tour/tourist/checkpoints')
  }
  ...
  addCheckpoints(tourId:number, checkpoints: Checkpoint[]): void {
    ...
    this.http.post<void>('http://localhost:8080/api/tour/'+ tourId + '/checkpoints' , checkpointsid).subscribe();
  }

  ...
  constructor(private authService: AuthService,
    ...
    getFullImageUrl(filePath: File | string | null | undefined): string {
      if (filePath && typeof filePath === 'string' && filePath.trim() !== '') {
        return `http://localhost:8080/Resources/Images/${filePath}`;
      } else if (filePath instanceof File && filePath.size !== 0) {
        return `http://localhost:8080/Resources/Images/${filePath.name}`;
      } 
    ...
    getFollowers():void{
      this.followersService.getFollowers(this.userId).subscribe({
        next: (ids: number[]) => {
          ...
          this.numOfFollowers = ids.length;
        }
      })
    }
    ...
    getFollowing():void{
      this.followersService.getFollowing(this.userId).subscribe({
        next: (ids: number[]) => {
          ...
          this.numOfFollowing = ids.length;
        }
      })
    }
    ...
    // Suggested profiles (recommendations) come from the Followers graph service.
        ...
        next: (ids: number[]) => {
          ...
          ids.forEach(id =>
            this.followersService.getUsername(id).subscribe({
              next: (u) => this.notFollowing.push({ id: u.id, username: u.username } as any)
            }));
        }
      });
    }

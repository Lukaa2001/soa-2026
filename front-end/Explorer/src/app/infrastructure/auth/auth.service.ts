...
export class AuthService {
  ...
  getNameById(id: number): Observable<People> {
    return this.http.get<People>('http://localhost:8080/api/person/'+id); 
  }

  updateUserLocation(person: People): Observable<People> {
    return this.http.put<People>('http://localhost:8080/api/person/updateLocation/', person);
  }

  getAllUsers(loggedId: number): Observable<PagedResults<User>> {
    return this.http.get<PagedResults<User>>(
      'http://localhost:8080/api/users/allUsers/GetAll/' + loggedId
    );
  }

  getUserById(id: number): Observable<User> {
    // SOA: resolve username via the gateway gRPC endpoint (gateway -> Stakeholders).
    return this.http.get<User>(environment.apiHost + 'rpc/users/' + id);
  }
}

...
public class TourExecutionService : BaseService<TourExecutionDto, TourExecution>, ITourExecutionService
{
    ...
    public Result<TourExecutionDto> Create(int tourId, long touristId)
    {
        try
        {
            ...
            // SOA: the purchase prerequisite (functionality 17) is enforced by the
            // Tours -> Purchase call in TourExecutionController, so the old in-process
            // ownership check (backed by a stubbed order service here) is skipped.

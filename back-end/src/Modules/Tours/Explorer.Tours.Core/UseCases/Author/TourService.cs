...
namespace Explorer.Tours.Core.UseCases.Author
{
    public class TourService : CrudService<TourDto, Tour>, ITourService, IInternalTourService
    {
        ...
        public Result<PagedResult<TourPreviewDto>> GetAllAvailableTours(int page, int pageSize, int userId)
        {
            ...
            // Only published tours are buyable (exclude drafts and archived).
            var dtos = result.Value.Results
                .Where(r => r.Status == Explorer.Tours.Core.Domain.Tours.Status.PUBLISHED)
                .Select(r => _mapper.Map<TourPreviewDto>(r)).ToList();
